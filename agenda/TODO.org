...
2) this is what casues the issue with the second pane shifting to another buffer. I have this in my dired config
...
I like to use the left and right keys to easly move to/from dirs (this is called lynx like motion in mc). are there similar SR command that do that?
...
4) I still cant get some options to work for me. mainly these (I set them all via customize-group as you suggested)

(sr-show-hidden-files t) 
SR still doesn't come up with dotfiles shown

5) for sr-show-files-info can the return info be in MB/GB (and not bytes :))
