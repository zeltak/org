...
** sunrise
-if only 1 match left in fuzzt match- auto select it (that is enter dir)
-how is checkpoint different from boomarks? the documentation is sparse on this point, when I issue sr-restore-checkpoint I still get my normal Emacs boomakrs there as well

Been coding all night? Enjoy the Sunrise! (or press q to quit)
Type "q" in help window to restore its previous buffer, <M-next> to scroll help.
Omitting...
(Nothing to omit)
Omitting...
(Nothing to omit)
