...
**** TODO Rasterize line layer
in qgis use:
Raster>>Conversion>>Rasterize
for input file choose the line layer
in the attribute field choose the ID for the lines output file doesn't exist. You must set up the output size or resolution to create it.
