...
file and import it using the method above.
 XLConnect
 still buggy but you may try it
...
Include the option append=TRUE to append text to the file rather than
overwriting it. Including the option split=TRUE will send output to both
the screen and the output file. Issuing the command sink() without
options will return output to the screen alone.
 IE:
 sink("myoutput", append=TRUE, split=TRUE)
 IE:

sink("c:\\Users\\ekloog\\Documents\\tmp\\rout.txt",append=TRUE,split=TRUE)
 TIP: you can use the text expansion tsink to expand in r
...
*assignments
*
 The returning value of an R function may be assigned to an R object to
store the results and permit other R functions to manipulate that result
object. An assignment also may copy all or a subset of an R object to
another R object.
 There are two possibilities how to assign a value to an object. Objects
obtain values in R by assignment. This is achieved by the gets arrow <-
which is a composite symbol made up from "less than" and "minus" with no
space between them. The other way of doing it is with the equal sign =.
I strongly recommend using the gets arrow instead of the equal sign,
because it's much less ambiguous. The equal sign could be misunderstood
as logical comparison ("is equal to").
 The expression to which the returned value is assigned may be a name, a
subset, an element, or an attribute of an R object. The following is the
assignment possible in R.
 expression <- value
 value -> expression
 The "<-" symbol should be read as "gets".
 Thus, to store the value 5 in the variable named a we type
 a <- 5 or a = 5
 This can be read as "assign the value 5 to a". To retrieve the value,
simply type in a and press Enter:
 a
 [1] 5
 To see the structure of a, type
 str(a)
 num 5
 The function str returns the internal structure of an R object. This
means, it displays (among other things) what variables an object
contains, how they are coded and how many observations there are. If you
are not sure, of what type your object is, use str to find it out.
 In the above example, R tells us that a is a numerical object (num) and
that a contains only a single number, 5. The important point here is
that you can store anything: a single scalar, a vector, a matrix, a data
frame, a model and so on.
 Assignments are actually made only upon the successful completion of
the function call. Should an error occur in the function, no assignment
will be made and the expression will not be altered. This protects data
objects from destruction during an erroneous function call.
 This feature may pose a problem when an assignment is used to create
intermediate variables within a function written by the user. These
intermediate variables do not take on the proper values until the entire
user function has completed execution and are therefore not available to
subsequent steps in that user function. However, there is a mandatory
assignment symbol that will cause an intermediate returning value to be
immediately saved as the expression:
 expression <<- value
 Users should not normally use the mandatory assignment symbol outside
of user functions. Within a user function, the mandatory assignment
symbol may have serious unintended consequences.

** Batch Processing

Batch Processing
 You can run R non-interactively with input from infile and send output
(stdout/stderr) to another file. Here are examples.
 # on linux
 R CMD BATCH [options] my\_script.R [outfile]
 # on ms windows (adjust the path to R.exe as needed)
 "C:\Program Files\R\R-2.5.0\bin\R.exe" CMD BATCH --vanilla --slave
"c:\my projects\my\_script.R"

** Customizing Startup

Customizing Startup
 You can customize the R environment through a site initialization file
or a directory initialization file.
 R will always source the *Rprofile.site* file first. On Windows, the
file is in the
 *C:\Program Files\R\R-n.n.n\etc\* directory.
 You can also place a .Rprofile file in any directory that you are going
to run R from or in the user home directory.
 At startup, R will source the Rprofile.site file. It will then look for
a .Rprofile file to source in the current working directory. If it
doesn't find it, it will look for one in the user's home directory.
There are two special functions you can place in these files. .First( )
will be run at the start of the R session and .Last( ) will be run at
...
Input/Output
 By default, launching R starts an interactive session with input from
the keyboard and output to the screen. However, you can have input come
from a script file (a file containing R commands) and direct output to a
...
use one of the following functions. Use dev.off( ) to return output to
the terminal.
 Function    Output to
 ...
 Use a full path in the file name to save the graph outside of the
...
Lists are the most complex of the R data types. Basically, a list is an
ordered collection of objects (components). A list allows you to gather
a variety of (possibly unrelated) objects under one name.
 For example, a list may contain a combination of vectors, matrices,
data frames, and even other lists. You create a list using the list()
function :
 mylist <- list(object1, object2, ...)
 where the objects are any of the structures seen so far. Optionally,
you can name the objects in a list: mylist <- list(name1=object1,
name2=object2, ...) The following listing shows an example.
 [[R_files/img/Image_t6aGFee2JPMvtKFtvyybxQ_0003.png]]
 In this example, you create a list with four components: a string, a
numeric vector, a matrix, and a character vector.
  You can combine any number of objects and save them as a list. You can
also specify elements of the list by indicating a component number or a
name within double brackets.
 In this example, mylist[[2]] and mylist[["ages"]] both refer to the
same four-element numeric vector. Lists are important R structures for
two reasons. First, they allow you to organize and recall disparate
information in a simple way. Second, the results of many R functions
return lists. It's up to the analyst to pull out the components that are
needed. You'll see numerous examples of functions that return lists in
later chapters.
...
*to delete missing cases from dataset based on a specific variables:*
 use:
 NEWDATA

= OLDDATA[complete.cases(OLDDATA$VARIABLEUSED),]

 #delete missing cases in predictions
 *T2000\_merged* =
*T2000\_merged*[complete.cases(*T2000\_merged*$*pred*),]
 where:
 *T2000\_merged-is the new data set without missing (in this case its
the old one without the missing)
 T2000\_merged-is the dataset with the missing
 pred-is the variable with the missing the removal will be based on
*
...
@unfinished section@ import the text from here:
...
*
 *example:*
 ICAM\_diab\_g <- ICAM\_diab[11:16,]
 Where:
...
■ statement is a single R statement or a compound statement (a group of
R statements enclosed in curly braces { } and separated by semicolons)
...
One of R's greatest strengths is the user's ability to add functions. In
fact, many of the
...
summary(nas1$bc\_pred[nas1$Year == 1995])
...
*
 The describe() function in the Hmisc package returns the number of
variables and observations, the number of missing and unique values, the
mean, quantiles, and the five highest and lowest values.
 *
*library(Hmisc)
 describe(mydata)
 # n, nmiss, unique, mean, 5,10,25,50,75,90,95th percentiles
 # 5 lowest and 5 highest scores
 [[R_files/img/Image_HHYtteFRaLyts49n0TiDog_0003.png]]
 *NOTE:* [[#DNZDHnrs0PMggw2EsCwRzA][see masking]] to address same name
function in packages

***** pastecs

*Using the pastecs package*
 The pastecs package includes a function named stat.desc() that provides
a wide range of descriptive statistics. The format is:
 *stat.desc(x, basic=TRUE, desc=TRUE, norm=FALSE, p=0.95)
*
 where x is a data frame or time series. If basic=TRUE (the default),
the number of values, null values, missing values, minimum, maximum,
range, and sum are provided. If desc=TRUE (also the default), the
median, mean, standard error of the mean, 95 per-
 cent confidence interval for the mean, variance, standard deviation,
and coefficient of variation are also provided. Finally, if norm=TRUE
(not the default), normal distribution statistics are returned,
including skewness and kurtosis (and their statistical signifi-
 cance), and the Shapiro--Wilk test of normality. A p-value option is
used to calculate the confidence interval for the mean (.95 by default).
Listing 7.4 gives an example.*
*
 [[R_files/img/Image_zYgybyVESc5i4LGhLkxzlQ_0003.png]]
 As if this isn't enough, the psych package also has a function called
describe() that provides the number of nonmissing observations, mean,
standard deviation, median, trimmed mean, median absolute deviation,
minimum, maximum, range, skew, kurtosis, and standard error of the mean.
You can see an example in the following listing.
 Examples:
 library(pastecs)
 stat.desc(mydata)
...
*Using the psych package is the recommended way for simple descriptive
statistics*
 library(psych)
 describe(mydata)
 [[R_files/img/Image_fsPo7A1n7M5ysB1jlyaBCA_0003.png]]
 *NOTE:* [[#DNZDHnrs0PMggw2EsCwRzA][see masking]] to address same name
function in packages
...
*NOTE:*
 The packages psych and Hmisc both provide functions named *describe()*.
Simply put, the package last loaded takes precedence.
 If psych is loaded after Hmisc, a message is printed indicating that
the describe() function in Hmisc is masked by the function in psych.
 When you type in the describe() function and R searches for it, R comes
to the psych package first and executes it. If you want the Hmisc
version instead, you can type
 Hmisc::describe(mt)
 The function is still there. You have to give R more information to
find it.
 ...
 *summaryBy(formula, data=dataframe, FUN=function)*
 ...
 var1 + var2 + var3 + ... + varN ~ groupvar1 + groupvar2 + ... +
groupvarN
 Variables on the left of the ~ are the*numeric variables to be
analyzed*
 Variables on the right are *categorical grouping variables*
 The function can be any [[#6WidyD3pF97CtIhGJoBD1g][built-in]] or user
created R function.
 It is possible use a vector of predefined functions. A typical usage
will be by invoking a list of predefined functions:
 ...
 *using functions:*
 library(doBy)
 # produces mean and SD for birth weight and mean30 for each level of
new\_race
 summaryBy( birthw+mean30~ new\_race, data = bw,
 ...
 *TABLE <- with(DATASET,table(CAT\_VAR))
*
...
table option useNA="ifany".*
 IE:
 mytable <- with(F\_T2000\_All, table(reg\_id))
 more example:
 [[R_files/img/Image_YKTjpQG4sEyGgwgcnK4gmA_0003.png]]
 code:
...
to omit missing values use:
 na.action=na.omit,
 IE:
 out.model\_zinc = lme( Znppm ~ CO +nitro+cultiv+iregg,
 random = ~1 | crop\_type, na.action=na.omit,
 data = zinc)
...
** lme4
*** extract from a mer object (random effect, cooefficents etc)

use the attributes function:
gm1x <- lmer( MFVALUE ~AOD+TEMP+t.avgs+s.avgs+st.avgs+(1+AOD+TEMP|DATE/reg_name),na.action=na.omit, data = F\_T2001\_All)
ranef.lme4 <- attributes(gm1x)$ranef
...
*** extract lme4 object to a table
...
from a set of continuous predictor variables. It is frequently preferred
over discriminant function analysis because of its less restrictive
assumptions.
 # Logistic Regression
...
density plot of the binary outcome F on the continuous x variable.
 [[R_files/img/Image_vMyDKmtVjOfGOxmWRHtnyw_0003.jpg]]
...
**** add residuals from GAM modles

 res\_2005 <- residuals(smooth\_T2005\_yearly[["gam"]])
...
family=gaussian(), data=hanesna)
 ...
 to assign a *random intercept+slope* to a model use this:
 random=list(VARIABLE=~1+Slope\_variable)
 *example:
*
 bp.model.gamm<-gamm(log(bp1sys)~ sex + as.factor(race) + smokpkyr +
bmi+ as.factor(booze) + age+ ser.chol, random=list(locode=~1+age),
family=gaussian(), data=hanesna)
 summary(bp.model.gamm$lme)
...
**** run a logistic (binomal) mixed model
 ...
 bp.model.gamm1<-gamm(d.total~ sex + as.factor(race) + smokpkyr +
s(bmi,bs='cr')+ as.factor(booze) + s(age,bs='cr')+
s(ser.chol,bs='cr')+s(bp1sys,bs='cr'), *random=list(locode=~1)*,
*family=binomial()*, data=hanesna)
...
**** create a celsius var based on farenhight
#+begin_src R
