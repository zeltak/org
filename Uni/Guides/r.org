...
#+END_SRC
$Note$  you adress the whole DF and not a specific variable 
...
** geomerge (nearestbyday)

@note-the id variable in the matirx must be character!!!@

₆In example₆ 
#+BEGIN_SRC R
s2x$GUID<-as.character(s2x$GUID)
aod.m <- makepointsmatrix(s2x[s2x[,unique(GUID)], list(Long, Lat, GUID), mult = "first"], "Long", "Lat", "GUID")
#+END_SRC
...
- knreast: number of nearest points it considers, if or if not they have data. assesment of how near something is , is done by pointmatrix. if you have many points with no data you might end up with no data.
its the maximum number of points that gets considered.
...
$Note$  knearest runs before maxdist, the maxdist is an additional screen
...
#+end_src@WARNING:
**** rena@me multiple variables/columns at once
#+begin_src R
