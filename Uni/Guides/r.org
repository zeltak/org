...
*** TODO the take first date in a sorted list
...
#+END_SRC
*** what to report in main cells

report: a  character  string  containing  only  elements  of

#+BEGIN_EXAMPLE
"v" , "c" , "s" , "t" , "p" , "*" 
#+END_EXAMPLE

that determines whether, and in which order, variable names ("v"), coefficients ("c"),  standard  errors/confidence  intervals  ("s"),  test  statistics  ("t")  and  p- values ("p") should be reported in regression tables.  If one of the aforementioned letters is followed by an asterisk ("*"), significance stars will be reported next to the corresponding sta
...
*** rbind in data.table
    :PROPERTIES:
    :ID:       34e7181a-bebb-4fe9-ae37-162d2c86edaa
    :END:
#+begin_src R
require(data.table)
...
*** create a date and guid 
#+BEGIN_SRC R :session Rorg  :results none
mod2000x<-mutate(mod2000x, day=as.Date(strptime(date, "%d%b%Y")))
mod2000x<-mutate(mod2000x,guid = paste( glong,glat,sep="") )
#+END_SRC
...
#+BEGIN_SRC R :session Rorg  :results none
bwfull<-select(bfull,ID,gestation,birth_weig,X,Y,dob,guid,GUID_1) %>% dplyr::rename (aodid=GUID_1)
...
#+BEGIN_SRC R
#+END_SRC
*** dply multi join by 2 or more variables, also different names
...
d3<-left_join(d1, d2, by = c("x" = "x2", "y" = "y2"))
#+END_SRC
