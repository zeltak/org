...
#+BEGIN_EXAMPLE
random=list(VARIABLE=~1+Slope_variable)
#+END_EXAMPLE
...
#+end_src
**** Join but dont keep all variables
example:
...
#+begin_src R
mod3 <- merge(mod3, pm[pm[, list(unique(day))],list(day,daymeanall,daymeanallnum),mult="first"], all.x = T)
#+end_src
**** Join with a subset of cases
#+begin_src R
midmod3grid <- merge(midmod3grid[.(c(as.Date("2003-01-01"), as.Date("2003-01-02"))),],mpm_agg[.(c(as.Date("2003-01-01"), as.Date("2003-01-02"))),],all.x=TRUE)
#+end_src
...
#+begin_src R
setnames(red.met, c("FECHA", "HORA"), c("day", "hour"))
#+end_src
*** rounding up data
**** round up lat/long
#+begin_src R
