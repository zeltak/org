...
data PM_2003(keep = PM25 Long_PM Lat_PM SiteCode EPAcode Date); 
 set PM.PM2003;
  where  '01jan2003'd <= date <= '31dec2003'd;  
  if pm25 < 0 then delete;
run;

data AOD_2003(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.All_aod;
  where  '01jan2003'd <= date <= '31dec2003'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2003 ***/

proc sort data = PM_2003 nodupkey out = PM_ID_2003(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2003 nodupkey out = PM_Date2003(keep=Date);     by Date;     run;

proc sort data = AOD_2003  nodupkey out = AOD_Date2003(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2003(in = a) AOD_Date2003(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2003 Pm_date2003 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2003;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2003;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2003'D and date<='31DEC2003'D ));
set results;
if aod=. then delete;
run; 
...
libname aod3 'c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\2.Gather_data\FN003_PM_AOD_Combined\' ;
