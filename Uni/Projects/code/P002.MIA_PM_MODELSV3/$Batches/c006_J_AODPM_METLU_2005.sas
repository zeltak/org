...
proc sort data =  Met2005; by date; run;

/**** Griglia STN ****/

proc freq data = Met2005; 
  table STN;
...
proc sort data = Met2005;   by date STN; run;
proc sort data = Final;     by date STN; run;

data Weather_2005;
 merge Met2005 (in=a) Final (in=b);   
  by date STN;
   if b;
run;
...
proc sort data =  Met2005 nodupkey out = STN_XY (keep = STN Lat_met Long_Met); by STN; run;
...
data st2005&ID; set STN_XY;
  where STN = &ID;
run;

data Dist_Met&ID;
  if _N_ = 1 then set st2005&ID;
   set STN_Fake;
...
proc datasets lib=work; delete id_elenco Dist_Met&ID st2005&ID; run;
...
proc means data = Met_Complete_2005 nmiss;
...
data metc.metc2005;
set Met_Complete_2005;
run; 

 

proc sort data = Aodpm2005         nodupkey out=Site_AODPM(keep=Sitecode Long_PM Lat_PM); by sitecode; run;



proc sort data = Met_complete_2005 nodupkey out=STN_2005(keep=STN Long_Met Lat_Met);     by Long_Met Lat_Met; run;
...
proc sort data = aodpm2005; by sitecode   ;run;
proc sort data = Res_aod_pm_met2 ; by sitecode ;run;

data aodpm2005_v2;
merge aodpm2005 (in=a) Res_aod_pm_met2 (in=b keep = sitecode stn)   ;
  by sitecode ;
    if a;
	run; 
...
proc sort data = aodpm2005_v2; by stn date   ;run;
proc sort data = Met_complete_2005 ; by stn date  ;run;

data aodpm2005_v3;
merge aodpm2005_v2 (in=a) Met_complete_2005 (in=b keep = stn date temp_f wdsp_f visib_f ah_gm3_F )   ;
  by stn date ;
    if a;
	run; 
...
proc sort data= Aodpm2005_v3;
by guid;
run;


data Aodpm2005_v4;
merge Aodpm2005_v3(in=a) lu (in=b keep=guid pop_sqkm elev A1_dist_km p_urban p_open area_pm point_pm population ) ;
by guid;
if a;
run;
...
proc sort data = Aodpm2005_v5; by guid   ;run;
proc sort data = guidreg ; by guid ;run;

data Aodpm2005_v6;
merge Aodpm2005_v5(in=a) guidreg (in=b keep=guid reg_id)  ;
  by guid;
    if a;
	run; 
...
data aodx.mod1_2005_prew;
set Aodpm2005_v6 ;
run;

/*save as csv for R*/

PROC EXPORT DATA= WORK.Aodpm2005_v6
            OUTFILE= "c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\3.Analysis\AN001_mod1\t2005.csv" 
			            DBMS=CSV REPLACE;
						     PUTNAMES=YES;
							 RUN;
