...
proc sort data =  Met2008; by date; run;

/**** Griglia STN ****/

proc freq data = Met2008; 
  table STN;
...
proc sort data = Met2008;   by date STN; run;
proc sort data = Final;     by date STN; run;

data Weather_2008;
 merge Met2008 (in=a) Final (in=b);   
  by date STN;
   if b;
run;
...
proc sort data =  Met2008 nodupkey out = STN_XY (keep = STN Lat_met Long_Met); by STN; run;
...
proc means data = Met_Complete_2008 nmiss;
...
data metc.metc2008;
set Met_Complete_2008;
run; 

 

proc sort data = Aodpm2008         nodupkey out=Site_AODPM(keep=Sitecode Long_PM Lat_PM); by sitecode; run;



proc sort data = Met_complete_2008 nodupkey out=STN_2008(keep=STN Long_Met Lat_Met);     by Long_Met Lat_Met; run;
...
proc sort data = aodpm2008; by sitecode   ;run;
proc sort data = Res_aod_pm_met2 ; by sitecode ;run;

data aodpm2008_v2;
merge aodpm2008 (in=a) Res_aod_pm_met2 (in=b keep = sitecode stn)   ;
  by sitecode ;
    if a;
	run; 
...
proc sort data = aodpm2008_v2; by stn date   ;run;
proc sort data = Met_complete_2008 ; by stn date  ;run;

data aodpm2008_v3;
merge aodpm2008_v2 (in=a) Met_complete_2008 (in=b keep = stn date temp_f wdsp_f visib_f ah_gm3_F )   ;
  by stn date ;
    if a;
	run; 
...
proc sort data= Aodpm2008_v3;
by guid;
run;


data Aodpm2008_v4;
merge Aodpm2008_v3(in=a) lu (in=b keep=guid pop_sqkm elev A1_dist_km p_urban p_open area_pm point_pm population ) ;
by guid;
if a;
run;
...
proc sort data = Aodpm2008_v5; by guid   ;run;
proc sort data = guidreg ; by guid ;run;

data Aodpm2008_v6;
merge Aodpm2008_v5(in=a) guidreg (in=b keep=guid reg_id)  ;
  by guid;
    if a;
	run; 
...
data aodx.mod1_2008_prew;
set Aodpm2008_v6 ;
run;

/*save as csv for R*/

PROC EXPORT DATA= WORK.Aodpm2008_v6
            OUTFILE= "c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\3.Analysis\AN001_mod1\t2008.csv" 
			            DBMS=CSV REPLACE;
						     PUTNAMES=YES;
							 RUN;
