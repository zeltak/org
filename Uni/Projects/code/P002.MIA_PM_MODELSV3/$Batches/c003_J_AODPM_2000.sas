...
data PM_2000(keep = PM25 Long_PM Lat_PM SiteCode EPAcode Date); 
 set PM.PM2000;
  where  '01mar2000'd <= date <= '31dec2000'd;  
  if pm25 < 0 then delete;
run;

data AOD_2000(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.All_aod;
  where  '01mar2000'd <= date <= '31dec2000'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2000 ***/

proc sort data = PM_2000 nodupkey out = PM_ID_2000(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2000 nodupkey out = PM_Date2000(keep=Date);     by Date;     run;

proc sort data = AOD_2000  nodupkey out = AOD_Date2000(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2000(in = a) AOD_Date2000(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2000 Pm_date2000 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2000;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2000;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2000'D and date<='31DEC2000'D ));
set results;
if aod=. then delete;
run; 
