...
data PM_2008(keep = PM25 Long_PM Lat_PM SiteCode EPAcode Date); 
 set PM.PM2008;
  where  '01jan2008'd <= date <= '31dec2008'd;  
  if pm25 < 0 then delete;
run;

data AOD_2008(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.All_aod;
  where  '01jan2008'd <= date <= '31dec2008'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2008 ***/

proc sort data = PM_2008 nodupkey out = PM_ID_2008(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2008 nodupkey out = PM_Date2008(keep=Date);     by Date;     run;

proc sort data = AOD_2008  nodupkey out = AOD_Date2008(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2008(in = a) AOD_Date2008(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2008 Pm_date2008 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2008;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2008;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2008'D and date<='31DEC2008'D ));
set results;
if aod=. then delete;
run; 
