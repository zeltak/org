...
data PM_2006(keep = PM25 Long_PM Lat_PM SiteCode EPAcode Date); 
 set PM.PM2006;
  where  '01jan2006'd <= date <= '31dec2006'd;  
  if pm25 < 0 then delete;
run;

data AOD_2006(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.All_aod;
  where  '01jan2006'd <= date <= '31dec2006'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2006 ***/

proc sort data = PM_2006 nodupkey out = PM_ID_2006(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2006 nodupkey out = PM_Date2006(keep=Date);     by Date;     run;

proc sort data = AOD_2006  nodupkey out = AOD_Date2006(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2006(in = a) AOD_Date2006(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2006 Pm_date2006 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2006;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2006;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2006'D and date<='31DEC2006'D ));
set results;
if aod=. then delete;
run; 
