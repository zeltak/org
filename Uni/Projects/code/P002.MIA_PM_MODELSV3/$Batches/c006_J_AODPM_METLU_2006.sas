libname aodpm 'c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\2.Gather_data\FN003_PM_AOD_Combined\' ;
...
proc sort data =  Met2006; by date; run;

/**** Griglia STN ****/

proc freq data = Met2006; 
  table STN;
...
proc sort data = Met2006;   by date STN; run;
proc sort data = Final;     by date STN; run;

data Weather_2006;
 merge Met2006 (in=a) Final (in=b);   
  by date STN;
   if b;
run;
...
proc sort data =  Met2006 nodupkey out = STN_XY (keep = STN Lat_met Long_Met); by STN; run;
...
data st2006&ID; set STN_XY;
  where STN = &ID;
run;

data Dist_Met&ID;
  if _N_ = 1 then set st2006&ID;
   set STN_Fake;
...
proc datasets lib=work; delete id_elenco Dist_Met&ID st2006&ID; run;
...
proc means data = Met_Complete_2006 nmiss;
...
data metc.metc2006;
set Met_Complete_2006;
run; 

 

proc sort data = Aodpm2006         nodupkey out=Site_AODPM(keep=Sitecode Long_PM Lat_PM); by sitecode; run;



proc sort data = Met_complete_2006 nodupkey out=STN_2006(keep=STN Long_Met Lat_Met);     by Long_Met Lat_Met; run;
...
proc sort data = aodpm2006; by sitecode   ;run;
proc sort data = Res_aod_pm_met2 ; by sitecode ;run;

data aodpm2006_v2;
merge aodpm2006 (in=a) Res_aod_pm_met2 (in=b keep = sitecode stn)   ;
  by sitecode ;
    if a;
	run; 
...
proc sort data = aodpm2006_v2; by stn date   ;run;
proc sort data = Met_complete_2006 ; by stn date  ;run;

data aodpm2006_v3;
merge aodpm2006_v2 (in=a) Met_complete_2006 (in=b keep = stn date temp_f wdsp_f visib_f ah_gm3_F )   ;
  by stn date ;
    if a;
	run; 
...
proc sort data= Aodpm2006_v3;
by guid;
run;


data Aodpm2006_v4;
merge Aodpm2006_v3(in=a) lu (in=b keep=guid pop_sqkm elev A1_dist_km p_urban p_open area_pm point_pm population ) ;
by guid;
if a;
run;
...
proc sort data = Aodpm2006_v5; by guid   ;run;
proc sort data = guidreg ; by guid ;run;

data Aodpm2006_v6;
merge Aodpm2006_v5(in=a) guidreg (in=b keep=guid reg_id)  ;
  by guid;
    if a;
	run; 
...
libname aodx 'c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\2.Gather_data\FN006_J_AODPM_METLU\' ;

data aodx.mod1_2006_prew;
set Aodpm2006_v6 ;
run;

/*save as csv for R*/

PROC EXPORT DATA= WORK.Aodpm2006_v6
            OUTFILE= "c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\3.Analysis\AN001_mod1\t2006.csv" 
			            DBMS=CSV REPLACE;
						     PUTNAMES=YES;
							 RUN;
...
libname guid_stn 'c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\2.Gather_data\FN007_Key_tables\' ;

data guid_stn.guid_stn_2006;
set Res_guid_met3 ;
run; 
