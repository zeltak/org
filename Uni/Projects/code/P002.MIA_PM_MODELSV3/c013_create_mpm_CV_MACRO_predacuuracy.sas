...
proc sort data = mpm&year._&it; by date reg_id   ;run;
proc sort data = mod2pred&year._&it ; by date reg_id ;run;

data mod2pred&year._V2;
merge mod2pred&year._&it(in=a) mpm&year._&it(in=b keep=date reg_id mpm_F)  ;
  by date reg_id;
    if a;
	run; 
...
PROC EXPORT DATA= WORK.mod2pred&year._V3
            OUTFILE= "c:\Users\ekloog\Documents\$Doc\3.PostDoc\3.1.Projetcs\3.1.2.MIA_PM_MODELSV3\3.1.1.4.Work\3.Analysis\AN009_mod2_CV_files_mpm_val\T&year._m2_pred_mpm_&it..csv" 
			            DBMS=CSV REPLACE;
						     PUTNAMES=YES;
							 RUN;
