...
#create full LU TS
days<-seq.Date(from = as.Date("2000-01-01"), to = as.Date("2000-12-31"), 1)
#create date range
aod2000 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2000,aodid,day)
setkey(aqua,aodid,day)
aodf.2000<-left_join(aod2000,aqua)

#add land use and X,Y
setkey(aodf.2000,aodid)
setkey(wlu,aodid)
aodf.2000.lu<-left_join(aodf.2000,wlu)
aodf.2000.lu<-aodf.2000.lu[, c(7:9) := NULL]
#clean points with no lu data (on borders and in golan)
aodf.2000.lu <- aodf.2000.lu[!is.na(pblid)]



#########-------------------############
#create temporal dataset
aodf.2000.tmp <-aodf.2000.lu[,c(1,2,9,10),with=FALSE]
...
met2000<- Temp[c==2000]
#create PM matrix
met.m <- makepointsmatrix(met2000, "X", "Y", "stn")
#create aod matrix
setkey(aodf.2000.tmp, aodid)
lu.m <- makepointsmatrix(aodf.2000.tmp[aodf.2000.tmp[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2000.tmp, met2000[, list(day,Temp,stn)], 
                            "aodid", "stn", "meanT", "Temp", knearest = 7, maxdistance = NA)

setkey(aodf.2000.tmp,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2000.tmp.s1 <- merge(aodf.2000.tmp, closestaodse[,list(day,Temp,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2000, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2000.tmp.s1, met2000[, list(day,WS,stn)], 
                            "aodid", "stn", "meanT", "WS", knearest = 7, maxdistance = NA)

setkey(aodf.2000.tmp.s1,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2000.tmp.s2 <- merge(aodf.2000.tmp.s1, closestaodse[,list(day,WS,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2000, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2000.tmp.s2, met2000[, list(day,WD,stn)], 
                            "aodid", "stn", "meanT", "WD", knearest = 7, maxdistance = NA)

setkey(aodf.2000.tmp.s2,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2000.tmp.s3 <- merge(aodf.2000.tmp.s2, closestaodse[,list(day,WD,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2000, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2000.tmp.s3, met2000[, list(day,Rain,stn)], 
                            "aodid", "stn", "meanT", "Rain", knearest = 7, maxdistance = NA)

setkey(aodf.2000.tmp.s3,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2000.tmp.s4 <- merge(aodf.2000.tmp.s3, closestaodse[,list(day,Rain,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2000, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2000.tmp.s4, met2000[, list(day,RH,stn)], 
                            "aodid", "stn", "meanT", "RH", knearest = 7, maxdistance = NA)

setkey(aodf.2000.tmp.s4,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2000.tmp.s4 <- merge(aodf.2000.tmp.s4, closestaodse[,list(day,RH,aodid)], all.x = T)


#join back LU
setkey(aodf.2000.lu,aodid,day)
setkey(aodf.2000.tmp.s4,aodid,day)
x1<- left_join(aodf.2000.lu,aodf.2000.tmp.s4)


#Join PBL
setkey(pbl , day, pblid)
setkey(x1, day, pblid)
aodf.2000.tmp.s6<-left_join(x1, pbl)


#add season
aodf.2000.tmp.s6$month <- as.numeric(format(aodf.2000.tmp.s6$day, "%m"))
#1-winter, 2-spring,3-summer,4-autum
aodf.2000.tmp.s6$season<-recode(aodf.2000.tmp.s6$month,"1=1;2=1;3=2;4=2;5=2;6=3;7=3;8=3;9=4;10=4;11=4;12=1")
#1-winter, 2-summer
aodf.2000.tmp.s6$seasonSW<-recode(aodf.2000.tmp.s6$month,"1=1;2=1;3=1;4=2;5=2;6=2;7=2;8=2;9=2;10=1;11=1;12=1")
#add month
aodf.2000.tmp.s6[, m := as.numeric(format(day, "%m")) ]
#add year
aodf.2000.tmp.s6[, c := as.numeric(format(day, "%Y")) ]
#join NDVI to aod
setkey(ndvi, ndviid, c, m )
setkey(aodf.2000.tmp.s6,  ndviid, c, m)
aodf.2000.tmp.s8 <- merge(aodf.2000.tmp.s6, ndvi, all.x = T)
...
setkey(aodf.2000.tmp.s8 , day, stn)
setkey(dust2, day, stn)
aodf.2000.tmp.s9 <- merge(aodf.2000.tmp.s8, dust2[,list(day,stn,Dust)], all.x = T)
aodf.2000.tmp.s9<-aodf.2000.tmp.s9[is.na(Dust), Dust:= 0]


#########-------------------############
#clean
aodf.2000.tmp.s9<-aodf.2000.tmp.s9[,]
aodf.2000.tmp.s9 <- aodf.2000.tmp.s9[Temp != 'NA']
#create weights
aodf.2000.tmp.s9<-aodf.2000.tmp.s9[,obs:=1]
aodf.2000.tmp.s9[is.na(aod), obs:= 0]
#model
w1<- glm(obs ~ elev+MeanPbl+Temp+as.factor(month),family=binomial,data=aodf.2000.tmp.s9)
aodf.2000.tmp.s9$prob <- predict(w1,type = c("response"))  #get probability prediction , note that its a binary logisitc and thus the type-repsonse option
aodf.2000.tmp.s9$wt <- 1/aodf.2000.tmp.s9$prob
aodf.2000.tmp.s9$normwt <- aodf.2000.tmp.s9$wt/mean(aodf.2000.tmp.s9$wt)
aodf.2000.tmp.s9<-aodf.2000.tmp.s9[, c(53,54) := NULL]
...
pmall2000<- PM25[c==2000]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pmall2000[,c("PM25","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pmall2000 <- pmall2000[pmall2000$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pmall2000, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2000.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2000.tmp.s9[aodf.2000.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2000.tmp.s9, pmall2000 [, list(day,PM25,stn)], 
...
#join to DB
setkey(closestaodse,aodid,day)
setkey(aodf.2000.tmp.s9,aodid,day)
aodf.2000.tmp.s9 <- merge(aodf.2000.tmp.s9,closestaodse,all.x = T)
...
pm10all2000<- PM10[c==2000]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pm10all2000[,c("PM10","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pm10all2000 <- pm10all2000[pm10all2000$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pm10all2000, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2000.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2000.tmp.s9[aodf.2000.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2000.tmp.s9, pm10all2000 [, list(day,PM10,stn)], 
...
###save mods
#mod3
saveRDS(aodf.2000.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2000.rds")
#mod2
aodf.2000.tmp.s9.m2 <- aodf.2000.tmp.s9[!is.na(aod)]
saveRDS(aodf.2000.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2000.rds")
#mod1

########### join aod to PM25
#create PM matrix
pm.m <- makepointsmatrix(PM25, "x_stn_ITM", "y_stn_ITM", "stn")
setkey(aodf.2000.tmp.s9.m2,aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2000.tmp.s9.m2[aodf.2000.tmp.s9.m2[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaod <- nearestbyday(pm.m, aod.m, 
                           PM25, aodf.2000.tmp.s9.m2, 
...
#save mod 1
saveRDS(PM25.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ.2000.rds")
...
#save mod 1
saveRDS(PM10.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ10.2000.rds")
...
#create full LU TS
days<-seq.Date(from = as.Date("2001-01-01"), to = as.Date("2001-12-31"), 1)
#create date range
aod2001 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2001,aodid,day)
setkey(aqua,aodid,day)
aodf.2001<-left_join(aod2001,aqua)

#add land use and X,Y
setkey(aodf.2001,aodid)
setkey(wlu,aodid)
aodf.2001.lu<-left_join(aodf.2001,wlu)
aodf.2001.lu<-aodf.2001.lu[, c(7:9) := NULL]
#clean points with no lu data (on borders and in golan)
aodf.2001.lu <- aodf.2001.lu[!is.na(pblid)]



#########-------------------############
#create temporal dataset
aodf.2001.tmp <-aodf.2001.lu[,c(1,2,9,10),with=FALSE]
...
met2001<- Temp[c==2001]
#create PM matrix
met.m <- makepointsmatrix(met2001, "X", "Y", "stn")
#create aod matrix
setkey(aodf.2001.tmp, aodid)
lu.m <- makepointsmatrix(aodf.2001.tmp[aodf.2001.tmp[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2001.tmp, met2001[, list(day,Temp,stn)], 
                            "aodid", "stn", "meanT", "Temp", knearest = 7, maxdistance = NA)

setkey(aodf.2001.tmp,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2001.tmp.s1 <- merge(aodf.2001.tmp, closestaodse[,list(day,Temp,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2001, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2001.tmp.s1, met2001[, list(day,WS,stn)], 
                            "aodid", "stn", "meanT", "WS", knearest = 7, maxdistance = NA)

setkey(aodf.2001.tmp.s1,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2001.tmp.s2 <- merge(aodf.2001.tmp.s1, closestaodse[,list(day,WS,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2001, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2001.tmp.s2, met2001[, list(day,WD,stn)], 
                            "aodid", "stn", "meanT", "WD", knearest = 7, maxdistance = NA)

setkey(aodf.2001.tmp.s2,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2001.tmp.s3 <- merge(aodf.2001.tmp.s2, closestaodse[,list(day,WD,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2001, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2001.tmp.s3, met2001[, list(day,Rain,stn)], 
                            "aodid", "stn", "meanT", "Rain", knearest = 7, maxdistance = NA)

setkey(aodf.2001.tmp.s3,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2001.tmp.s4 <- merge(aodf.2001.tmp.s3, closestaodse[,list(day,Rain,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2001, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2001.tmp.s4, met2001[, list(day,RH,stn)], 
                            "aodid", "stn", "meanT", "RH", knearest = 7, maxdistance = NA)

setkey(aodf.2001.tmp.s4,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2001.tmp.s4 <- merge(aodf.2001.tmp.s4, closestaodse[,list(day,RH,aodid)], all.x = T)


#join back LU
setkey(aodf.2001.lu,aodid,day)
setkey(aodf.2001.tmp.s4,aodid,day)
x1<- left_join(aodf.2001.lu,aodf.2001.tmp.s4)


#Join PBL
setkey(pbl , day, pblid)
setkey(x1, day, pblid)
aodf.2001.tmp.s6<-left_join(x1, pbl)


#add season
aodf.2001.tmp.s6$month <- as.numeric(format(aodf.2001.tmp.s6$day, "%m"))
#1-winter, 2-spring,3-summer,4-autum
aodf.2001.tmp.s6$season<-recode(aodf.2001.tmp.s6$month,"1=1;2=1;3=2;4=2;5=2;6=3;7=3;8=3;9=4;10=4;11=4;12=1")
#1-winter, 2-summer
aodf.2001.tmp.s6$seasonSW<-recode(aodf.2001.tmp.s6$month,"1=1;2=1;3=1;4=2;5=2;6=2;7=2;8=2;9=2;10=1;11=1;12=1")
#add month
aodf.2001.tmp.s6[, m := as.numeric(format(day, "%m")) ]
#add year
aodf.2001.tmp.s6[, c := as.numeric(format(day, "%Y")) ]
#join NDVI to aod
setkey(ndvi, ndviid, c, m )
setkey(aodf.2001.tmp.s6,  ndviid, c, m)
aodf.2001.tmp.s8 <- merge(aodf.2001.tmp.s6, ndvi, all.x = T)
...
setkey(aodf.2001.tmp.s8 , day, stn)
setkey(dust2, day, stn)
aodf.2001.tmp.s9 <- merge(aodf.2001.tmp.s8, dust2[,list(day,stn,Dust)], all.x = T)
aodf.2001.tmp.s9<-aodf.2001.tmp.s9[is.na(Dust), Dust:= 0]


#########-------------------############
#clean
aodf.2001.tmp.s9<-aodf.2001.tmp.s9[,]
aodf.2001.tmp.s9 <- aodf.2001.tmp.s9[Temp != 'NA']
#create weights
aodf.2001.tmp.s9<-aodf.2001.tmp.s9[,obs:=1]
aodf.2001.tmp.s9[is.na(aod), obs:= 0]
#model
w1<- glm(obs ~ elev+MeanPbl+Temp+as.factor(month),family=binomial,data=aodf.2001.tmp.s9)
aodf.2001.tmp.s9$prob <- predict(w1,type = c("response"))  #get probability prediction , note that its a binary logisitc and thus the type-repsonse option
aodf.2001.tmp.s9$wt <- 1/aodf.2001.tmp.s9$prob
aodf.2001.tmp.s9$normwt <- aodf.2001.tmp.s9$wt/mean(aodf.2001.tmp.s9$wt)
aodf.2001.tmp.s9<-aodf.2001.tmp.s9[, c(53,54) := NULL]
...
setnames(PM25,"X","x_stn_ITM")
setnames(PM25,"Y","y_stn_ITM")
pmall2001<- PM25[c==2001]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pmall2001[,c("PM25","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pmall2001 <- pmall2001[pmall2001$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pmall2001, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2001.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2001.tmp.s9[aodf.2001.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2001.tmp.s9, pmall2001 [, list(day,PM25,stn)], 
                            "aodid", "stn", "meanPM25", "PM25", knearest = 6, maxdistance = NA)
#check data completness
x1<-closestaodse[, .N, by=c("aodid")]
summary(x1)
...
#join to DB
setkey(closestaodse,aodid,day)
setkey(aodf.2001.tmp.s9,aodid,day)
aodf.2001.tmp.s9 <- merge(aodf.2001.tmp.s9,closestaodse,all.x = T)
...
setnames(PM10,"X","x_stn_ITM")
setnames(PM10,"Y","y_stn_ITM")
pm10all2001<- PM10[c==2001]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pm10all2001[,c("PM10","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pm10all2001 <- pm10all2001[pm10all2001$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pm10all2001, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2001.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2001.tmp.s9[aodf.2001.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2001.tmp.s9, pm10all2001 [, list(day,PM10,stn)], 
                            "aodid", "stn", "meanPM10", "PM10", knearest = 6, maxdistance = NA)
#check data completness
x1<-closestaodse[, .N, by=c("aodid")]
summary(x1)
...
###save mods
#mod3
saveRDS(aodf.2001.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2001.rds")
#mod2
aodf.2001.tmp.s9.m2 <- aodf.2001.tmp.s9[!is.na(aod)]
saveRDS(aodf.2001.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2001.rds")
#mod1

########### join aod to PM25
#create PM matrix
pm.m <- makepointsmatrix(PM25, "x_stn_ITM", "y_stn_ITM", "stn")
setkey(aodf.2001.tmp.s9.m2,aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2001.tmp.s9.m2[aodf.2001.tmp.s9.m2[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaod <- nearestbyday(pm.m, aod.m, 
                           PM25, aodf.2001.tmp.s9.m2, 
                           "stn", "aodid", "closestaod", "aod", knearest = 7, maxdistance = 1500)
closestaod[,i.stn :=NULL]
closestaod[,closestaodknn :=NULL]
closestaod[,closestaodnobs:=NULL]
...
PM25.m1<-PM25.m1[!is.na(aod)]
#save mod 1
saveRDS(PM25.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ.2001.rds")
...
PM10.m1<-PM10.m1[!is.na(aod)]
#save mod 1
saveRDS(PM10.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ10.2001.rds")
...
#create date range
aod2002 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2002,aodid,day)
setkey(aqua,aodid,day)
aodf.2002<-left_join(aod2002,aqua)

#add land use and X,Y
setkey(aodf.2002,aodid)
setkey(wlu,aodid)
aodf.2002.lu<-left_join(aodf.2002,wlu)
aodf.2002.lu<-aodf.2002.lu[, c(7:9) := NULL]
#clean points with no lu data (on borders and in golan)
aodf.2002.lu <- aodf.2002.lu[!is.na(pblid)]



#########-------------------############
#create temporal dataset
aodf.2002.tmp <-aodf.2002.lu[,c(1,2,9,10),with=FALSE]
...
met2002<- Temp[c==2002]
#create PM matrix
met.m <- makepointsmatrix(met2002, "X", "Y", "stn")
#create aod matrix
setkey(aodf.2002.tmp, aodid)
lu.m <- makepointsmatrix(aodf.2002.tmp[aodf.2002.tmp[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2002.tmp, met2002[, list(day,Temp,stn)], 
                            "aodid", "stn", "meanT", "Temp", knearest = 7, maxdistance = NA)

setkey(aodf.2002.tmp,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2002.tmp.s1 <- merge(aodf.2002.tmp, closestaodse[,list(day,Temp,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2002, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2002.tmp.s1, met2002[, list(day,WS,stn)], 
                            "aodid", "stn", "meanT", "WS", knearest = 7, maxdistance = NA)

setkey(aodf.2002.tmp.s1,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2002.tmp.s2 <- merge(aodf.2002.tmp.s1, closestaodse[,list(day,WS,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2002, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2002.tmp.s2, met2002[, list(day,WD,stn)], 
                            "aodid", "stn", "meanT", "WD", knearest = 7, maxdistance = NA)

setkey(aodf.2002.tmp.s2,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2002.tmp.s3 <- merge(aodf.2002.tmp.s2, closestaodse[,list(day,WD,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2002, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2002.tmp.s3, met2002[, list(day,Rain,stn)], 
                            "aodid", "stn", "meanT", "Rain", knearest = 7, maxdistance = NA)

setkey(aodf.2002.tmp.s3,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2002.tmp.s4 <- merge(aodf.2002.tmp.s3, closestaodse[,list(day,Rain,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2002, "X", "Y", "stn")
closestaodse<- nearestbydayMEAN(lu.m ,met.m , 
                            aodf.2002.tmp.s4, met2002[, list(day,RH,stn)], 
                            "aodid", "stn", "meanT", "RH", knearest = 7, maxdistance = NA)

setkey(aodf.2002.tmp.s4,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2002.tmp.s4 <- merge(aodf.2002.tmp.s4, closestaodse[,list(day,RH,aodid)], all.x = T)


#join back LU
setkey(aodf.2002.lu,aodid,day)
setkey(aodf.2002.tmp.s4,aodid,day)
x1<- left_join(aodf.2002.lu,aodf.2002.tmp.s4)


#Join PBL
setkey(pbl , day, pblid)
setkey(x1, day, pblid)
aodf.2002.tmp.s6<-left_join(x1, pbl)


#add season
aodf.2002.tmp.s6$month <- as.numeric(format(aodf.2002.tmp.s6$day, "%m"))
#1-winter, 2-spring,3-summer,4-autum
aodf.2002.tmp.s6$season<-recode(aodf.2002.tmp.s6$month,"1=1;2=1;3=2;4=2;5=2;6=3;7=3;8=3;9=4;10=4;11=4;12=1")
#1-winter, 2-summer
aodf.2002.tmp.s6$seasonSW<-recode(aodf.2002.tmp.s6$month,"1=1;2=1;3=1;4=2;5=2;6=2;7=2;8=2;9=2;10=1;11=1;12=1")
#add month
aodf.2002.tmp.s6[, m := as.numeric(format(day, "%m")) ]
#add year
aodf.2002.tmp.s6[, c := as.numeric(format(day, "%Y")) ]
#join NDVI to aod
setkey(ndvi, ndviid, c, m )
setkey(aodf.2002.tmp.s6,  ndviid, c, m)
aodf.2002.tmp.s8 <- merge(aodf.2002.tmp.s6, ndvi, all.x = T)
...
setkey(aodf.2002.tmp.s8 , day, stn)
setkey(dust2, day, stn)
aodf.2002.tmp.s9 <- merge(aodf.2002.tmp.s8, dust2[,list(day,stn,Dust)], all.x = T)
aodf.2002.tmp.s9<-aodf.2002.tmp.s9[is.na(Dust), Dust:= 0]


#########-------------------############
#clean
aodf.2002.tmp.s9<-aodf.2002.tmp.s9[,]
aodf.2002.tmp.s9 <- aodf.2002.tmp.s9[Temp != 'NA']
#create weights
aodf.2002.tmp.s9<-aodf.2002.tmp.s9[,obs:=1]
aodf.2002.tmp.s9[is.na(aod), obs:= 0]
#model
w1<- glm(obs ~ elev+MeanPbl+Temp+as.factor(month),family=binomial,data=aodf.2002.tmp.s9)
aodf.2002.tmp.s9$prob <- predict(w1,type = c("response"))  #get probability prediction , note that its a binary logisitc and thus the type-repsonse option
aodf.2002.tmp.s9$wt <- 1/aodf.2002.tmp.s9$prob
aodf.2002.tmp.s9$normwt <- aodf.2002.tmp.s9$wt/mean(aodf.2002.tmp.s9$wt)
aodf.2002.tmp.s9<-aodf.2002.tmp.s9[, c(53,54) := NULL]
...
setnames(PM25,"X","x_stn_ITM")
setnames(PM25,"Y","y_stn_ITM")
pmall2002<- PM25[c==2002]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pmall2002[,c("PM25","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pmall2002 <- pmall2002[pmall2002$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pmall2002, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2002.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2002.tmp.s9[aodf.2002.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2002.tmp.s9, pmall2002 [, list(day,PM25,stn)], 
                            "aodid", "stn", "meanPM25", "PM25", knearest = 6, maxdistance = NA)
#check data completness
x1<-closestaodse[, .N, by=c("aodid")]
summary(x1)
...
#join to DB
setkey(closestaodse,aodid,day)
setkey(aodf.2002.tmp.s9,aodid,day)
aodf.2002.tmp.s9 <- merge(aodf.2002.tmp.s9,closestaodse,all.x = T)
...
setnames(PM10,"X","x_stn_ITM")
setnames(PM10,"Y","y_stn_ITM")
pm10all2002<- PM10[c==2002]
#keep only full stations
table_temp<-as.data.table(ddply(na.omit(pm10all2002[,c("PM10","stn"),with=F]),.(stn),nrow))
table_temp<-table_temp[V1 > 364]
pm10all2002 <- pm10all2002[pm10all2002$stn %in% table_temp$stn, ] 
#create PM matrix
pm.m <- makepointsmatrix(pm10all2002, "x_stn_ITM", "y_stn_ITM", "stn")
### create aod grid
setkey(aodf.2002.tmp.s9, aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2002.tmp.s9[aodf.2002.tmp.s9[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaodse<- nearestbydayMEAN(aod.m  ,pm.m , 
                            aodf.2002.tmp.s9, pm10all2002 [, list(day,PM10,stn)], 
                            "aodid", "stn", "meanPM10", "PM10", knearest = 6, maxdistance = NA)
#check data completness
x1<-closestaodse[, .N, by=c("aodid")]
summary(x1)
...
###save mods
#mod3
saveRDS(aodf.2002.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2002.rds")
#mod2
aodf.2002.tmp.s9.m2 <- aodf.2002.tmp.s9[!is.na(aod)]
saveRDS(aodf.2002.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2002.rds")
#mod1

########### join aod to PM25
#create PM matrix
pm.m <- makepointsmatrix(PM25, "x_stn_ITM", "y_stn_ITM", "stn")
setkey(aodf.2002.tmp.s9.m2,aodid)
#create aod terra matrix
aod.m <- makepointsmatrix(aodf.2002.tmp.s9.m2[aodf.2002.tmp.s9.m2[,unique(aodid)], list(x_aod_ITM, y_aod_ITM, aodid), mult = "first"], "x_aod_ITM", "y_aod_ITM", "aodid")
closestaod <- nearestbyday(pm.m, aod.m, 
                           PM25, aodf.2002.tmp.s9.m2, 
                           "stn", "aodid", "closestaod", "aod", knearest = 7, maxdistance = 1500)
closestaod[,i.stn :=NULL]
closestaod[,closestaodknn :=NULL]
closestaod[,closestaodnobs:=NULL]
...
PM25.m1<-PM25.m1[!is.na(aod)]
#save mod 1
saveRDS(PM25.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ.2002.rds")
...
PM10.m1<-PM10.m1[!is.na(aod)]
#save mod 1
saveRDS(PM10.m1,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod1.AQ10.2002.rds")
...
#create date range
aod2004 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2004,aodid,day)
setkey(aqua,aodid,day)
aodf.2004<-left_join(aod2004,aqua)
...
###save mods
#mod3
saveRDS(aodf.2004.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2004.rds")
#mod2
...
#create date range
aod2005 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2005,aodid,day)
setkey(aqua,aodid,day)
aodf.2005<-left_join(aod2005,aqua)
...
###save mods
#mod3
saveRDS(aodf.2005.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2005.rds")
#mod2
aodf.2005.tmp.s9.m2 <- aodf.2005.tmp.s9[!is.na(aod)]
saveRDS(aodf.2005.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2005.rds")
#mod1
...
#create date range
aod2006 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2006,aodid,day)
setkey(aqua,aodid,day)
aodf.2006<-left_join(aod2006,aqua)
...
###save mods
#mod3
saveRDS(aodf.2006.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2006.rds")
#mod2
...
#create date range
aod2007 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2007,aodid,day)
setkey(aqua,aodid,day)
aodf.2007<-left_join(aod2007,aqua)
...
###save mods
#mod3
saveRDS(aodf.2007.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2007.rds")
#mod2
aodf.2007.tmp.s9.m2 <- aodf.2007.tmp.s9[!is.na(aod)]
saveRDS(aodf.2007.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2007.rds")
#mod1
...
#create date range
aod2008 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2008,aodid,day)
setkey(aqua,aodid,day)
aodf.2008<-left_join(aod2008,aqua)
...
###save mods
#mod3
saveRDS(aodf.2008.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2008.rds")
#mod2
aodf.2008.tmp.s9.m2 <- aodf.2008.tmp.s9[!is.na(aod)]
saveRDS(aodf.2008.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2008.rds")
#mod1
...
#create date range
aod2009 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2009,aodid,day)
setkey(aqua,aodid,day)
aodf.2009<-left_join(aod2009,aqua)
...
###save mods
#mod3
saveRDS(aodf.2009.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2009.rds")
#mod2
aodf.2009.tmp.s9.m2 <- aodf.2009.tmp.s9[!is.na(aod)]
saveRDS(aodf.2009.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2009.rds")
#mod1
...
#create date range
aod2010 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2010,aodid,day)
setkey(aqua,aodid,day)
aodf.2010<-left_join(aod2010,aqua)
...
###save mods
#mod3
saveRDS(aodf.2010.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2010.rds")
#mod2
aodf.2010.tmp.s9.m2 <- aodf.2010.tmp.s9[!is.na(aod)]
saveRDS(aodf.2010.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2010.rds")
#mod1
...
#create date range
aod2012 <- data.table(expand.grid(aodid = aqua[, unique(aodid)], day = days))
setkey(aod2012,aodid,day)
setkey(aqua,aodid,day)
aodf.2012<-left_join(aod2012,aqua)
...
###save mods
#mod3
saveRDS(aodf.2012.tmp.s9,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod3.AQ.2012.rds")
#mod2
aodf.2012.tmp.s9.m2 <- aodf.2012.tmp.s9[!is.na(aod)]
saveRDS(aodf.2012.tmp.s9.m2,"/media/NAS/Uni/Projects/P046_Israel_MAIAC/3.Work/2.Gather_data/FN000_RWORKDIR/mod2.AQ.2012.rds")
#mod1
