...
setkey(closestaodse,aodid,day)
aodf.2003.tmp.s2 <- merge(aodf.2003.tmp.s1, closestaodse[,list(day,Temp,aodid)], all.x = T)
...
setkey(closestaodse,aodid,day)
aodf.2003.tmp.s3 <- merge(aodf.2003.tmp.s2, closestaodse[,list(day,Temp,aodid)], all.x = T)
...
met.m <- makepointsmatrix(met2003, "X", "Y", "stn")
closestaodse<- nearestbyday(lu.m ,met.m , 
                            aodf.2003.tmp.s3, met2003[, list(day,NO2,stn)], 
                            "aodid", "stn", "meanT", "NO2", knearest = 5, maxdistance = NA)

setkey(aodf.2003.tmp.s3,aodid,day)
setkey(closestaodse,aodid,day)
aodf.2003.tmp.s4 <- merge(aodf.2003.tmp.s3, closestaodse[,list(day,Temp,aodid)], all.x = T)
