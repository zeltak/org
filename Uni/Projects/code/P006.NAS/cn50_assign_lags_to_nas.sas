...
PROC IMPORT OUT= WORK.nasguid
              DATAFILE= "P:\3.1.6.NAS\3.1.6.4.Work\2.Gather_data\FN007_key\nas_tempguid.dbf" 
            DBMS=DBF REPLACE;
     GETDELETED=NO;
RUN;

data  nasguid (drop = xnym ynym xnymx ynymx);;;
set  nasguid;
...
libname exp 'P:\3.1.6.NAS\3.1.6.4.Work\2.Gather_data\FN031_exposure\' ;
...
proc sort data = expo; by guid date   ;run;
proc sort data = add_days2z ; by guid date ;run;

data add_days3;
merge add_days2z(in=a) expo (in=b  )  ;
  by guid date;
    if a;
	run; 
...
%local i;
 ...
 &pol.mayear = mean(of &pol._l0 - &pol._l364);
 run;
%mend;

%makelags(add_days3x,fintemp);



data fintemp_s;
set fintemp;
drop fintemp_l21--fintemp_l366;
run; 
...
data fintemp_s (drop=nasid);
set fintemp_s;
nasid1=nasid*1;
run; 

data fintemp_s(drop=nasid1);
set fintemp_s;
nasid=nasid1;
run; 

proc sort data = pmnew_shortZ3; by nasid   ;run;
proc sort data = fintemp_s ; by  nasid ;run;

data  fintemp_short_fin;
merge fintemp_s(in=a) pmnew_shortZ3 (in=b keep= nasid flag1)  ;
  by  nasid;
    if a;
	run; 
...
/*	load full nas dataset*/
	...
	libname nas 'P:\3.1.6.NAS\3.1.6.1.Raw_data\ORIG_NAS_2013\' ;
...
proc sort data = fintemp_short_fin; by nasid date   ;run;
proc sort data = nas_short ; by nasid date ;run;

data nas_short_temp;
merge nas_short(in=a) fintemp_short_fin(in=b)  ;
  by nasid date;
    if a;
	run; 
...
libname fin 'P:\3.1.6.NAS\3.1.6.4.Work\2.Gather_data\FN040_temp_final\' ;


data fin.Final_NAS_temp;
set nas_short_temp_clean;
run; 


data fin.Final_NAS_temp;
set fin.Final_NAS_temp;
fintemp_ma48 = mean(fintemp_l1,fintemp_l2);
run; 
