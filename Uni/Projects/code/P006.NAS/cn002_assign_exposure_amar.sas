...
data add_days3;
merge add_days2z(in=a) expo (in=b keep=addid date pmnew temp_f )  ;
  by addid date;
    if a;
	run; 
...
data DATA3;
merge pmnew_short(in=a)  temp_f_short (in=b) ;
  by addid date;
    if a;
	run; 
...
data fin.Final_NAS_POLL_lpm_amar;
