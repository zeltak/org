 function [vNightData] = ReadNightData (filename) 
    ...
    NightData(NightData == fillvalue) = NaN;

    % Process valid_range.
    NightData(NightData < valid_range(1)) = NaN;
    NightData(NightData > valid_range(2)) = NaN;

    % Apply scale factor according to the field attribute LST.
    % "LST: LST data * scale_factor".
    NightData = NightData * scale;
    ...
    [nrowNightData,ncolNightData] = size(NightData);

    %Reshape from matrix to linear vector
    vNightData = reshape(NightData,nrowNightData*ncolNightData,1);
