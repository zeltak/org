...
# 
ranlung = read.csv("c:/Users/ekloog/Documents/$Doc/3.PostDoc/3.1.Projetcs/3.1.12.MORTALITY_NE/3.1.10.4.Work/3.Analysis/AN001_R_files_bycase/cvdmort_lung.csv", header=T) 
...
try1_cvd <- (glmmPQL(count ~ ns(date,df=45)+deltapm1+mpm1+temp_f_l1+med_inc+Avg_per_mi+Avg_pctcol+Avg_p_A65, random = ~ 1 | guid, family = poisson, data = ts0004lagcv))
...
try1_st<- (glmmPQL(count ~ ns(date,df=45)+deltapm1+mpm1+temp_f_l1+med_inc+Avg_per_mi+Avg_pctcol+Avg_p_A65, random = ~ 1 | guid, family = poisson, data = ts0004lagst))
...
try1_st<- (glmmPQL(count ~ ns(date,df=45)+deltapm1+mpm1+temp_f_l1+med_inc+Avg_per_mi+Avg_pctcol+Avg_p_A65, random = ~ 1 | guid, family = poisson, data = ts0004lagres))
summary(try1_st)$tTable
...
try1_diab<- (glmmPQL(count ~ ns(date,df=45)+deltapm1+mpm1+temp_f_l1+med_inc+Avg_per_mi+Avg_pctcol+Avg_p_A65, random = ~ 1 | guid, family = poisson, data = ts0004lagdiab))
...
try1_lung<- (glmmPQL(count ~ ns(date,df=45)+deltapm1+mpm1+temp_f_l1+med_inc+Avg_per_mi+Avg_pctcol+Avg_p_A65, random = ~ 1 | guid, family = poisson, data = ts0004laglung))
