...
#create CV table
mod1table <- data.frame(type=character(19), R2=numeric(19),Bias=numeric(19),RMSPE=numeric(19))
#name columns
mod1table$type <- c("WV", "VA", "PA", "MD", "NY", "RI", "NJ", "DE", "CT", "MA", "VT", "NH", "ME","Boston","NYC","rural","urban","winter","summer")
...
bosoutd = lme(tempc ~ NTckin+elev+purban+NDVI+ws, random = ~1 + NTckin| date,  data =  mod1d) 
...
nyoutd = lme(tempc ~ NTckin+elev+purban+NDVI+ws, random = ~1 + NTckin| date,  data =  mod1d) 
...
#urban
m3pred2005_urb <- mod1d[purban > 22.2222, ]
m3pred2005_rural <- mod1d[purban <=22.2222, ]
...
mod1d_winteroutd = lme(tempc ~ NTckin+elev+purban+NDVI+ws, random = ~1 + NTckin| date,  data =  mod1d_winter) 
mod1d_winter$predicted <- predict(object=mod1d_winteroutd,newdata=mod1d_winter )
mod1d_winter_out<-lm(tempc~predicted,data=mod1d_winter)
mod1table$R2[18] <-summary(mod1d_winter_out)$r.squared
mod1table$Bias[18] <-summary(mod1d_winter_out)$coef[2,1]
#rmspe
mod1table$RMSPE[18]<- sqrt(mean(mod1d_winter_out$residual^2))


mod1d_summeroutd = lme(tempc ~ NTckin+elev+purban+NDVI+ws, random = ~1 + NTckin| date,  data =  mod1d_summer) 
mod1d_summer$predicted <- predict(object=mod1d_summeroutd,newdata=mod1d_summer )
mod1d_summer_out<-lm(tempc~predicted,data=mod1d_summer)
mod1table$R2[19] <-summary(mod1d_summer_out)$r.squared
mod1table$Bias[19] <-summary(mod1d_summer_out)$coef[2,1]
#rmspe
mod1table$RMSPE[19]<- sqrt(mean(mod1d_summer_out$residual^2))
