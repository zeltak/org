...
###GAMM
m1.formula <- as.formula(tm ~ s(Longitude,Latitude)+elev_m+pcturb +(1|date)) 

# Fitting a model and plotting it:
modx <- gamm4(tm ~elev_m+pcturb, data=m1.all, random = ~ (1|date))

mody <- gamm4(tm ~s(Longitude,Latitude)+elev_m+pcturb, data=m1.all, random = ~ (1|date))
...
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P022_Temprature_France/3.work/cn099.R/NORS_mod1cv.ALL.rds")
