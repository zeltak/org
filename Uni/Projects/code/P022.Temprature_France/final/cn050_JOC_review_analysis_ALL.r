...
fi1_gam <- gam( tm ~ s(Longitude,Latitude,by=dd), data=m1.all.cv)
fi1_gam <- gam( tm ~ s(Longitude,Latitude)+elev_m+pcturb+NDVI, data=m1.all.cv)
summary(fi1_gam)
m1.all.cv$pred<- predict(fi1_gam)
