...
MOD13A3 <- "ftp://e4ftl01.cr.usgs.gov/MOLT/MOD09Q1.005/"
#MOD13A3a <- "ftp://anonymous:test@e4ftl01.cr.usgs.gov/MOLT/MOD09A1.005/"
...
BLOCK1 <- getlist[grep(getlist, pattern="MOD09Q1.*.h20v05.*.hdf")[1]]
BLOCK2 <- getlist[grep(getlist, pattern="MOD09Q1.*.h21v05.*.hdf")[1]]
BLOCK3 <- getlist[grep(getlist, pattern="MOD09Q1.*.h20v06.*.hdf")[1]]
BLOCK4 <- getlist[grep(getlist, pattern="MOD09Q1.*.h21v06.*.hdf")[1]]
...
download.file(paste(MOD13A3, dates$dirname[[i]], "/", BLOCK1,sep=""), destfile=paste(getwd(), "/", BLOCK1,sep=""), mode='wb', method='wget', quiet=T, cacheOK=FALSE)
...
shell(cmd=paste(MRT, 'mrtmosaic -i ', MRT, 'TmpMosaic.prm -o ', workd, 'TmpMosaic.hdf', sep=""))
...
write('  ', filename, append=TRUE)
write('SPATIAL_SUBSET_UL_CORNER = ( 615964.0 3691819.0 )', filename, append=TRUE)
write('SPATIAL_SUBSET_LR_CORNER = ( 770622.0 3456429.0 )', filename, append=TRUE)
write('  ', filename, append=TRUE)
