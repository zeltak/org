...
libname xo 'P:\P042_Medicare_DVT\3.1.10.4.Work\3.Analysis\AN001_CasesXover\' ;
libname full 'P:\P042_Medicare_DVT\3.1.10.1.Raw_data\PM\' ;
...
proc phreg data=xo.try_all_sub35  nosummary;
    model time*case(0) = pmnewma1 temp_fma1 sex age wd1--wd6;
    strata  QID  date;
	ods output ParameterEstimates=pmnewma1;
		run;
    ...
    strata  QID  date;
	ods output ParameterEstimates=xo.pmnewma3;
		run;
...
proc phreg data=xo.try_all nosummary;
    model time*case(0) = pmnewmaweek temp_fmaweek  wd1--wd6 age sex race;
    strata  QID  date;
	ods output ParameterEstimates=xo.pmnewmaweek;
	run;
...
proc phreg data=xo.try_all nosummary;
    ...
    strata  QID  date;
	ods output ParameterEstimates=xo.pmnew_l0;
	run;
...
PROC IMPORT OUT= x1
            DATAFILE= "P:\P042_Medicare_DVT\3.1.10.4.Work\2.Gather_data\FN002_Cases_guid_MIA\cases_DVT_clipped_XY.dbf" 
			            DBMS=DBF   REPLACE;
						     GETDELETED=NO;
							 RUN; 
...
libname xor 'P:\P042_Medicare_DVT\3.1.10.5.Results\xover\' ;
