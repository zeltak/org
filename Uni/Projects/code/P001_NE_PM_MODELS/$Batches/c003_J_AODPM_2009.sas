...
data PM_2009(keep = PM25 Long_PM Lat_PM SiteCode  Date); 
 set PM.PM2009;
  where  '01jan2009'd <= date <= '31dec2009'd;  
  if pm25 < 0 then delete;
run;

data AOD_2009(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.aod_2009;
  where  '01jan2009'd <= date <= '31dec2009'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2009 ***/

proc sort data = PM_2009 nodupkey out = PM_ID_2009(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2009 nodupkey out = PM_Date2009(keep=Date);     by Date;     run;

proc sort data = AOD_2009  nodupkey out = AOD_Date2009(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2009(in = a) AOD_Date2009(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2009 Pm_date2009 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2009;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2009;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2009'D and date<='31DEC2009'D ));
set results;
if aod=. then delete;
run; 
