...
data PM_2010(keep = PM25 Long_PM Lat_PM SiteCode  Date); 
 set PM.PM2010;
  where  '01jan2010'd <= date <= '31dec2010'd;  
  if pm25 < 0 then delete;
run;

data AOD_2010(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.aod_2010;
  where  '01jan2010'd <= date <= '31dec2010'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2010 ***/

proc sort data = PM_2010 nodupkey out = PM_ID_2010(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2010 nodupkey out = PM_Date2010(keep=Date);     by Date;     run;

proc sort data = AOD_2010  nodupkey out = AOD_Date2010(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2010(in = a) AOD_Date2010(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2010 Pm_date2010 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2010;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2010;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2010'D and date<='31DEC2010'D ));
set results;
if aod=. then delete;
run; 
