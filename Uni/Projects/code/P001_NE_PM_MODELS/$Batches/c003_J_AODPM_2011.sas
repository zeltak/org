...
data PM_2011(keep = PM25 Long_PM Lat_PM SiteCode  Date); 
 set PM.PM2011;
  where  '01jan2011'd <= date <= '31dec2011'd;  
  if pm25 < 0 then delete;
run;

data AOD_2011(keep = Long_AOD Lat_AOD AOD Date);
 set AOD.aod_2011;
  where  '01jan2011'd <= date <= '31dec2011'd;  
run;

/*** Number of SiteCode -- Number of Dates available, Year:2011 ***/

proc sort data = PM_2011 nodupkey out = PM_ID_2011(keep=SiteCode);  by SiteCode; run; 
proc sort data = PM_2011 nodupkey out = PM_Date2011(keep=Date);     by Date;     run;

proc sort data = AOD_2011  nodupkey out = AOD_Date2011(keep=Date); by Date; run;   

/*** Merge by Common Date ***/

data Common_Date; merge PM_Date2011(in = a) AOD_Date2011(in = b); by date; if a and b; run;
...
PROC DATASETS LIBRARY=work; delete id_list Aod_date2011 Pm_date2011 Date_comuni; RUN;
...
data PM_01_&Date;   set PM_2011;   where date = &Date; run;
data AOD_01_&Date;  set AOD_2011;  where date = &Date; run;
...
data resultsv2(where=(date>='01JAN2011'D and date<='31DEC2011'D ));
set results;
if aod=. then delete;
run; 
