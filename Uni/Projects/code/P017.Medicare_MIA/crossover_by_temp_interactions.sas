...
data try2;
set try;
if temp_f < 17.1 then tb=0; 
if  17.1 <= temp_f <= 75.1 then tb=1;
if temp_f > 75.1 then tb=2; 
cenpm = pmnewma1 -13.5727921;
if race=1 then brace=1; *white;
if race = 3 then brace=2; *black;
if brace = . then brace=3;
run; 
...
proc means data=try2 n min max mean std nmiss;
var age; 
where tb=0;
run;
...
/*	ods output ParameterEstimates=mypar2 (keep= parameter Estimate  StdErr);*/
	...
	class tb;
    model time*case(0) = cenpm temp_f cenpm*tb tb  wd1--wd6;
    strata  hic  date;
/*	where tb=1;*/
	run;

data aod.try_cvd_controls;
set try;
run; 
...
/*	class tb;*/
	...
	proc phreg data=try2 nosummary;
	class tb;
    model time*case(0) = cenpm temp_f cenpm*tb tb tb*temp_f  wd1--wd6;
    strata  hic  date;
	run;
