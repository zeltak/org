...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2013.rds")
...
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2013.rds")
...
m2.all.2013<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2013 )
...
setkey(m2.all.2013,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2013  )
...
#------------------------>>>foo
#import mod3 
data.m3.2013  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2013.rds")
#for PM25
data.m3.2013 <- select(data.m3.2013,day,GUID,Month,meanPM,Long,Lat)
data.m3.2013[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2013,day, GUID)
...
setkey(data.m3.2013_bimon1,GUID)
data.m3.2013_bimon1 <- merge(data.m3.2013_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2013_bimon2,GUID)
data.m3.2013_bimon2 <- merge(data.m3.2013_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2013_bimon3,GUID)
data.m3.2013_bimon3 <- merge(data.m3.2013_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2013_bimon4,GUID)
data.m3.2013_bimon4 <- merge(data.m3.2013_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2013_bimon5,GUID)
data.m3.2013_bimon5 <- merge(data.m3.2013_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2013_bimon6,GUID)
data.m3.2013_bimon6 <- merge(data.m3.2013_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2013.pred3.rds")
