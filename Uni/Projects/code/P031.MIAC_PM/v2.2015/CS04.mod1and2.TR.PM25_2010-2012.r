...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2011.rds")
...
#BIND 1 dataset
m1.all.cv<- data.table(rbind(test_s1,test_s2,test_s3,test_s4,test_s5,test_s6,test_s7,test_s8,test_s9, test_s10))
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2011.rds")
# cleanup (remove from WS) objects from CV
rm(list = ls(pattern = "train_|test_"))
#table updates
m1.fit.all.cv<-lm(PM25~pred.m1.cv,data=m1.all.cv)
...
m2.all[, bimon := (Month + 1) %/% 2]
setkey(m2.all,day, GUID)
m2.all.2011<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
#run the lmer part regressing stage 2 pred Vs mean pm
#in israel check per month, also check 30km band and other methods for meanpm
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2011 )
#correlate to see everything from mod2 and the mpm works
...
#this is important so that its sorted as in the first gamm
setkey(m2.all.2011,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2011  )
...
#------------------------>>>foo
#import mod3 
data.m3.2011  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2011.rds")
#for PM25
data.m3.2011 <- select(data.m3.2011,day,GUID,Month,meanPM,Long,Lat)
data.m3.2011[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2011,day, GUID)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> merges
setkey(uniq_gid_bimon1,GUID)
setkey(data.m3.2011_bimon1,GUID)
data.m3.2011_bimon1 <- merge(data.m3.2011_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2011_bimon2,GUID)
data.m3.2011_bimon2 <- merge(data.m3.2011_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2011_bimon3,GUID)
data.m3.2011_bimon3 <- merge(data.m3.2011_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2011_bimon4,GUID)
data.m3.2011_bimon4 <- merge(data.m3.2011_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2011_bimon5,GUID)
data.m3.2011_bimon5 <- merge(data.m3.2011_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2011_bimon6,GUID)
data.m3.2011_bimon6 <- merge(data.m3.2011_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#recode negative into zero
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2011.pred3.rds")
...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2012.rds")
...
#BIND 1 dataset
m1.all.cv<- data.table(rbind(test_s1,test_s2,test_s3,test_s4,test_s5,test_s6,test_s7,test_s8,test_s9, test_s10))
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2012.rds")
# cleanup (remove from WS) objects from CV
rm(list = ls(pattern = "train_|test_"))
#table updates
m1.fit.all.cv<-lm(PM25~pred.m1.cv,data=m1.all.cv)
...
m2.all[, bimon := (Month + 1) %/% 2]
setkey(m2.all,day, GUID)
m2.all.2012<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
#run the lmer part regressing stage 2 pred Vs mean pm
#in israel check per month, also check 30km band and other methods for meanpm
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2012 )
#correlate to see everything from mod2 and the mpm works
...
#this is important so that its sorted as in the first gamm
setkey(m2.all.2012,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2012  )
...
#------------------------>>>foo
#import mod3 
data.m3.2012  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2012.rds")
#for PM25
data.m3.2012 <- select(data.m3.2012,day,GUID,Month,meanPM,Long,Lat)
data.m3.2012[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2012,day, GUID)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> merges
setkey(uniq_gid_bimon1,GUID)
setkey(data.m3.2012_bimon1,GUID)
data.m3.2012_bimon1 <- merge(data.m3.2012_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2012_bimon2,GUID)
data.m3.2012_bimon2 <- merge(data.m3.2012_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2012_bimon3,GUID)
data.m3.2012_bimon3 <- merge(data.m3.2012_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2012_bimon4,GUID)
data.m3.2012_bimon4 <- merge(data.m3.2012_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2012_bimon5,GUID)
data.m3.2012_bimon5 <- merge(data.m3.2012_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2012_bimon6,GUID)
data.m3.2012_bimon6 <- merge(data.m3.2012_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#recode negative into zero
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2012.pred3.rds")
