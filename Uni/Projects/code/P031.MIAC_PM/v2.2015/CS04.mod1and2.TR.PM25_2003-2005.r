...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2003.rds")
...
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2003.rds")
...
m2.all.2003<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2003 )
...
setkey(m2.all.2003,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2003  )
...
#------------------------>>>foo
#import mod3 
data.m3.2003  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2003.rds")
#for PM25
data.m3.2003 <- select(data.m3.2003,day,GUID,Month,meanPM,Long,Lat)
data.m3.2003[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2003,day, GUID)
...
setkey(data.m3.2003_bimon1,GUID)
data.m3.2003_bimon1 <- merge(data.m3.2003_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2003_bimon2,GUID)
data.m3.2003_bimon2 <- merge(data.m3.2003_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2003_bimon3,GUID)
data.m3.2003_bimon3 <- merge(data.m3.2003_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2003_bimon4,GUID)
data.m3.2003_bimon4 <- merge(data.m3.2003_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2003_bimon5,GUID)
data.m3.2003_bimon5 <- merge(data.m3.2003_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2003_bimon6,GUID)
data.m3.2003_bimon6 <- merge(data.m3.2003_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2003.pred3.rds")
...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2004.rds")
...
#BIND 1 dataset
m1.all.cv<- data.table(rbind(test_s1,test_s2,test_s3,test_s4,test_s5,test_s6,test_s7,test_s8,test_s9, test_s10))
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2004.rds")
# cleanup (remove from WS) objects from CV
rm(list = ls(pattern = "train_|test_"))
#table updates
m1.fit.all.cv<-lm(PM25~pred.m1.cv,data=m1.all.cv)
...
m2.all[, bimon := (Month + 1) %/% 2]
setkey(m2.all,day, GUID)
m2.all.2004<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
#run the lmer part regressing stage 2 pred Vs mean pm
#in israel check per month, also check 30km band and other methods for meanpm
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2004 )
#correlate to see everything from mod2 and the mpm works
...
#this is important so that its sorted as in the first gamm
setkey(m2.all.2004,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2004  )
...
#------------------------>>>foo
#import mod3 
data.m3.2004  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2004.rds")
#for PM25
data.m3.2004 <- select(data.m3.2004,day,GUID,Month,meanPM,Long,Lat)
data.m3.2004[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2004,day, GUID)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> merges
setkey(uniq_gid_bimon1,GUID)
setkey(data.m3.2004_bimon1,GUID)
data.m3.2004_bimon1 <- merge(data.m3.2004_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2004_bimon2,GUID)
data.m3.2004_bimon2 <- merge(data.m3.2004_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2004_bimon3,GUID)
data.m3.2004_bimon3 <- merge(data.m3.2004_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2004_bimon4,GUID)
data.m3.2004_bimon4 <- merge(data.m3.2004_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2004_bimon5,GUID)
data.m3.2004_bimon5 <- merge(data.m3.2004_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2004_bimon6,GUID)
data.m3.2004_bimon6 <- merge(data.m3.2004_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#recode negative into zero
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2004.pred3.rds")
...
saveRDS(m1.all,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1C.TR.PM25.pred.2005.rds")
...
#BIND 1 dataset
m1.all.cv<- data.table(rbind(test_s1,test_s2,test_s3,test_s4,test_s5,test_s6,test_s7,test_s8,test_s9, test_s10))
saveRDS(m1.all.cv,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod1cv.TR.PM25.2005.rds")
# cleanup (remove from WS) objects from CV
rm(list = ls(pattern = "train_|test_"))
#table updates
m1.fit.all.cv<-lm(PM25~pred.m1.cv,data=m1.all.cv)
...
m2.all[, bimon := (Month + 1) %/% 2]
setkey(m2.all,day, GUID)
m2.all.2005<-m2.all[!is.na(meanPM)]
rm(m2.all)
gc()
...
#run the lmer part regressing stage 2 pred Vs mean pm
#in israel check per month, also check 30km band and other methods for meanpm
m2.smooth <- lme(pred.m2 ~ meanPM,random = list(GUID= ~1 + meanPM),control=lmeControl(opt = "optim"), data= m2.all.2005 )
#correlate to see everything from mod2 and the mpm works
...
#this is important so that its sorted as in the first gamm
setkey(m2.all.2005,day, GUID)

#rerun the lme on the predictions including the spatial spline (smooth)
Final_pred_all <- lme(pred.t32 ~ meanPM ,random = list(GUID= ~1 + meanPM ),control=lmeControl(opt = "optim"),data= m2.all.2005  )
...
#------------------------>>>foo
#import mod3 
data.m3.2005  <- readRDS("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/Xmod3.Tr.2005.rds")
#for PM25
data.m3.2005 <- select(data.m3.2005,day,GUID,Month,meanPM,Long,Lat)
data.m3.2005[, bimon := (Month + 1) %/% 2]
setkey(data.m3.2005,day, GUID)
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> merges
setkey(uniq_gid_bimon1,GUID)
setkey(data.m3.2005_bimon1,GUID)
data.m3.2005_bimon1 <- merge(data.m3.2005_bimon1, uniq_gid_bimon1[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon2,GUID)
setkey(data.m3.2005_bimon2,GUID)
data.m3.2005_bimon2 <- merge(data.m3.2005_bimon2, uniq_gid_bimon2[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon3,GUID)
setkey(data.m3.2005_bimon3,GUID)
data.m3.2005_bimon3 <- merge(data.m3.2005_bimon3, uniq_gid_bimon3[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon4,GUID)
setkey(data.m3.2005_bimon4,GUID)
data.m3.2005_bimon4 <- merge(data.m3.2005_bimon4, uniq_gid_bimon4[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon5,GUID)
setkey(data.m3.2005_bimon5,GUID)
data.m3.2005_bimon5 <- merge(data.m3.2005_bimon5, uniq_gid_bimon5[,list(GUID,gpred)], all.x = T)
setkey(uniq_gid_bimon6,GUID)
setkey(data.m3.2005_bimon6,GUID)
data.m3.2005_bimon6 <- merge(data.m3.2005_bimon6, uniq_gid_bimon6[,list(GUID,gpred)], all.x = T)
...
#recode negative into zero
#mod3 <- mod3[pred.m3 >= 0]
saveRDS(mod3,"/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/mod3.TR.PM25.2005.pred3.rds")
