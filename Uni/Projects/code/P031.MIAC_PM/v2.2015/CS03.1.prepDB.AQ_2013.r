...
PM25 <- fread("/media/NAS/Uni/Projects/P031_MIAC_PM/3.Work/2.Gather_data/m3rds/pm25_northeast_2013_matlab.csv")
PM25 [PM25  == -999] <- NA
...
#setnames(PM25.2013,"sitecode","SiteCode")
g<- PM25.2013 %>%
    group_by(SiteCode,day) %>%
    summarise(avg_delay = mean(PM25, na.rm=TRUE))
PM25.2013$SiteCode<-as.character(PM25.2013$SiteCode)
...
setkey(PM25.2013,SiteCode,day)
setkey(closestaod,SiteCode,day)
PM25.m1 <- merge(PM25.2013[,list(SiteCode,day,PM25)], closestaod, all.x = T)
