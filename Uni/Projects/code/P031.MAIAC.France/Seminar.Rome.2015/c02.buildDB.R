...
aqua.met <- nearestbyday(lu.m ,met.m , 
                            ...
                            "aodid", "stn", "cloesetSTN", "tempavg", knearest = 10, maxdistance = 20000)
...
aqua.met <- nearestbyday(lu.m ,met.m , 
                            ...
                            "aodid", "stn", "cloesetSTN", "wsavg", knearest = 10, maxdistance = 20000)
setkey(db2003,aodid,day)
setkey(aqua.met,aodid,day)
...
pmj1<- nearestbyday(aod.m  ,pm.m , 
                            ...
                            "aodid", "stn", "closest","pm25",knearest = 10, maxdistance = 60000, nearestmean = T)
#join to DB
...
pmj1<- nearestbyday(aod.m  ,pm.m , 
                            ...
                            "aodid", "stn", "closest","pm10",knearest = 10, maxdistance = 60000, nearestmean = T)
gc()
#join to DB
setkey(pmj1,aodid,day)
setkey(db2003,aodid,day)
...
w1 <- glm(obs ~ elev_m+tempc+ws+PBL+as.factor(m),family=binomial,data=ws.2003)
ws.2003$prob <- predict(w1 ,type = c("response"))  
ws.2003$wt <- 1/ws.2003$prob
ws.2003$normwt <- ws.2003$wt/mean(ws.2003$wt)
ws.2003[, c("prob", "wt","obs","elev_m", "PBL" , "m","tempavg"  ) := NULL]
gc()

#reread m3 and join weights
db2003<-readRDS("/media/NAS/Uni/Projects/P031_MAIAC_France/2.work/WORKDIR/mod3.AQ.2003.rds")
setkey(db2003,aodid,day)
setkey(ws.2003,aoodid,day)
db2003 <- merge(db2003,ws.2003,all.x = T)
