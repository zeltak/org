...
#load data
#mod1 <-readRDS("/media/NAS/Uni/Projects/P031_MAIAC_France/2.work/WORKDIR/mod1.AQ.2006.PM25.clean.rds")
mod1i <-readRDS("/media/NAS/Uni/Projects/P031_MAIAC_France/2.work/WORKDIR/mod1.AQ.2006.PM25imp.clean.rds")
...
###base model
m1.raw.formula <- as.formula(pm25~ aod)
m1_sc <- lm(m1.raw.formula,data=mod1)
summary(m1_sc)
...
###base model-i
m1.raw.formula <- as.formula(PM25~ aod)
m1_sc <- lm(m1.raw.formula,data=mod1i)
summary(m1_sc)
...
mod1i<-filter(mod1i,flag==1)

###base modeli
m1.formula <- as.formula(PM25~ aod+(1+aod|day))
m1_sc <- lmer(m1.formula,data=mod1i)
summary(m1_sc)
mod1i$pred.m1 <- predict(m1_sc)
print(summary(lm(PM25~pred.m1,data=mod1i))$r.squared)
#0.65 real PM
...
#traffc data
+distA1
#+tden+dair+dport+dist_train+dist.aroad+dist.coast+dist.wb
#land use
+pcturb+p.agric+p.open+p.urban+p.forest
#random component
+(1+aod|day/cid))  
####
m1_sc <- lmer(m1.formula,data=mod1)
...
x<-gam(pm25~s(distA1)+s(pcturb)+s(p.urban),data=mod1)
plot(x)
...
m1.formula <- as.formula(PM25new ~ aod
#spline
#+ns(BLH,8)
#temporal
+temp+ndvi+ws+TP
...
summary(m1_sc)
mod1$pred.m1 <- predict(m1_sc)
res[res$type=="pm25", 'm1.R2'] <- print(summary(lm(PM25new~pred.m1,data=mod1))$r.squared)
...
res[res$type=="pm25", 'm1.R2'] <- print(summary(lm(PM25new~pred.m1,data=mod1))$r.squared)
    ...
    spatialall<-mod1 %>%
        group_by(SiteName) %>%
        dplyr::summarise(barpm = mean(PM25new, na.rm=TRUE), barpred = mean(pred.m1, na.rm=TRUE)) 
...
tempoall<-left_join(mod1,spatialall)
    tempoall$delpm <-tempoall$PM25new-tempoall$barpm
...
#save
 ...
 #---------------->>>> CV
      ...
      #table updates
      m1.fit.all.cv<-lm(PM25new~pred.m1.cv,data=mod1.cv)
      res[res$type=="pm25", 'm1cv.R2'] <- print(summary(lm(PM25new~pred.m1.cv,data=mod1.cv))$r.squared)
      res[res$type=="pm25", 'm1cv.I'] <-print(summary(lm(PM25new~pred.m1.cv,data=mod1.cv))$coef[1,1])
      res[res$type=="pm25", 'm1cv.Ise'] <-print(summary(lm(PM25new~pred.m1.cv,data=mod1.cv))$coef[1,2])
      res[res$type=="pm25", 'm1cv.slope'] <-print(summary(lm(PM25new~pred.m1.cv,data=mod1.cv))$coef[2,1])
      res[res$type=="pm25", 'm1cv.slopese'] <-print(summary(lm(PM25new~pred.m1.cv,data=mod1.cv))$coef[2,2])
      #RMSPE
      res[res$type=="pm25", 'm1cv.rmspe'] <- print(rmse(residuals(m1.fit.all.cv)))
      #spatial
      spatialall.cv<-mod1.cv %>%
          group_by(SiteName) %>%
          summarise(barpm = mean(PM25new, na.rm=TRUE), barpred = mean(pred.m1, na.rm=TRUE)) 
      ...
      tempoall.cv$delpm <-tempoall.cv$PM25new-tempoall.cv$barpm
...
res[res$type=="PM25", 'm1cvloc.R2'] <- print(summary(lm(PM25new~pred.m1.bothsvm,data=mod1.cv))$r.squared)
res[res$type=="PM25", 'm1cvloc.Ise'] <-print(summary(lm(PM25new~pred.m1.bothsvm,data=mod1.cv))$coef[1,2])
res[res$type=="PM25", 'm1cvloc.slope'] <-print(summary(lm(PM25new~pred.m1.bothsvm,data=mod1.cv))$coef[2,1])
res[res$type=="PM25", 'm1cvloc.slopese'] <-print(summary(lm(PM25new~pred.m1.bothsvm,data=mod1.cv))$coef[2,2])
#RMSPE
m1.fit.all.loc<-lm(PM25new~pred.m1.bothsvm,data=mod1.cv)
res[res$type=="PM25", 'm1cvloc.rmspe'] <- print(rmse(residuals(m1.fit.all.loc)))

#spatial
spatialall.cv.loc<-mod1.cv %>%
    group_by(SiteName) %>%
    summarise(barpm = mean(PM25new, na.rm=TRUE), barpred = mean(pred.m1.bothsvm, na.rm=TRUE)) 
...
tempoall.loc.cv$delpm <-tempoall.loc.cv$PM25new-tempoall.loc.cv$barpm
