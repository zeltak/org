...
[Hum-soc-staff] 
[Academia-IL] 
no-reply@researchgate.net 
meniba@bgu.ac.il
noreply@academia-mail.com
...
**** How to Avoid Google.com Redirects When You’re Outside the U.S.
Here’s how you can avoid that redirection.

1. Visit google.com/?gws_rd=ssl instead of google.com to head straight to the standardized version of Google.

Bookmark this modified URL or use an auto-expander to type it in quickly with a keyword of your choice. 
*** isp
...
***** NEW- AS only modem- tapouch 7- 1.1.2016			     :ATTACH:
      :PROPERTIES:
      :Attachments: VDSL_bezeq_modem.pdf
      :ID:       2bfe78ba-078d-459e-8d07-d49f7c3d4f27
      :END:
make sure the modem is only a modem without router functionality (see attached pdf to this header)
this us due to concatenating routers which always casues issue on same IP netowrk
...
הוספת קישור 
WAN PPP
: תחת אותו תפריט יש ללחוץ על פקד "
Add
"
←
בחירה 
...
then configure the wifi router to connect through PPPoE
***** OLD-AS A ROUTER 12.6.2014
...
@If your router doesnt have a WAN connection@
1. you must use the "do not use external ip check" to yes
...
2.make sure you have a local DNS setting in setup filled
***** Removing the DD-WRT Software
http://vpnpick.com/install-configure-dd-wrt/
...
#+END_SRC
***** upgrade dd-wrt
If DD-WRT is already installed on your router, you can simply upgrade to a new version via the web interface or TFTP.
However, it is highly recommended that you restore the router to defaults using the 30/30/30 reset
after the 30/30/30 reset you can just upgrade the router through the web interface
!Never restore old backups from previous versions! 
Skipping these steps could lead to a bricked device!
***** current setup dlink 25.12.2015
****** setup
WAN Connection Type
Connection Type
Optional Settings
...
Unmask
Hostname
zeltak.mooo.com,aUx3anhFYU5KZ1d0N2gwckw4TWc6MTE2NTI4ODA=
Use external ip checkYes No
...
Physical Interface wl0 - SSID [zdlink-5g] HWAddr [54:B8:0A:AA:86:9A]
Wireless Mode
...
Physical Interface wl1 - SSID [zdlink-2.4g] HWAddr [54:B8:0A:AA:86:98]
Wireless Mode
Wireless Network Mode
Wireless Network Name (SSID)
zdlink-2.4g
...
10.0.0.2
**** dlink dir-880l
***** restore original firmware from recovery mode 
...
8.After flashing let the router boot up.....now hold the reset button until the power LED goes red and let the unit reboot with a clear config.
Now use your web browser and configure the unit as if it was new.
***** reset to factory values
hold the reset button until the power LED goes red and let the unit reboot with a clear config
