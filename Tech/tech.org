...
**** gmail\_lifehacker\_tips.pdf

[[tech_files/attach/gmail_lifehacker_tips.pdf][Attachment #03
(gmail\_lifehacker\_tips.pdf)]]
...
*Zeltak IP Table*
 Host/Device name (Static IP Address) (Local services)
  
  Voices (Main Quad 6600)
  192.168.0.1 Marvin (Media server TNG)
  192.168.0.3 ZTPC (Living room HTPC)
  192.168.0.7
  xbmc web interface
  192.168.0.7:8083 Michal (Shanti)   
  192.168.0.14 BTPC (bedroom HTPC)
  192.168.0.3 Zwind
  192.168.0.33 OR Dhcp (101 usually) TP-Link Router/Firewall
  192.168.0.44Belkin Access Point
 192.168.0.93 Eddie (Media server) RETIRED
 192.168.0.2

*** misc addresses-info

Shahar's home IP address:
 192.115.133.122
 Netvision server IP
 212.143.205.253
 Setup bezeq simple network with ADSL modem
 Network card (connected to the hub/netwrok):
  
 -192.168.0.1
 -255.255.255.0
 -Leave gateway blank!
  
 Modem card (connected to ADSL modem)
  
 -10.0.0.2
 -255.0.0.0
 -Leave gateway blank!
 -DNS: 192.115.106.10 (11 alt)
...
**** DNSMasq Local Network ('sync' dynamic IP/local IP)
