...
use
M-; (comment-dwim)
**** set the type of comment manually
use 
#+BEGIN_SRC emacs-lisp  :results none
(setq comment-start "#")
#+END_SRC
...
** Tramp
*** how can I use sudo when I want to edit files on the server

#+BEGIN_SRC emacs-lisp  :results none
/ssh:user@machine|sudo:machine:/path/to/file 
#+END_SRC

let you edit files with root credentials on the other machine, after first login as user
** text tricks
...
[[http://johnmacfarlane.net/pandoc/demo/example27.org][http://johnmacfarlane.net/pandoc/demo/example27.org]]
 Warning if you get an error open the html file in an editor and
save it with utf-8 encoding
*** convert org headers to numbers
